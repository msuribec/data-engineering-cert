...
class ProgressStore:
    """Persist one user's progress in SQLite locally or Postgres when deployed."""
    ...
    def _sql(self, query: str) -> str:
        return query if self.backend == "sqlite" else query.replace("?", "%s")
    ...
    def get_card_progress(self, card_ids: Sequence[str]) -> dict[str, dict[str, Any]]:
        ...
        with self._connect() as connection:
            for offset in range(0, len(card_ids), 800):
                batch = list(card_ids[offset : offset + 800])
        ...
        *,
        ...
        with self._connect() as connection:
            ...
            for question_id in incorrect_ids:
                source_quiz_id, source_section = sources.get(question_id, (quiz_id, section))
                self._execute(
                    connection,
                    """
                    ...
                    """,
                    (
                        self.user_id, question_id, source_quiz_id, source_section,
                        to_iso(attempted_at), to_iso(attempted_at),
                    ),
                )
            for question_id in correct_ids:
                self._execute(
                    ...
                    """,
                    (to_iso(attempted_at), self.user_id, question_id),
                )
            return attempt_id
    ...
    def dashboard_stats(
        ...
        with self._connect() as connection:
            total_reviewed = self._execute(
                connection,
                """
                SELECT COUNT(*) AS count FROM study_card_progress
            ...
            due = self._execute(
                ...
                """,
                (self.user_id, to_iso(current)),
            ).fetchone()["count"]
            quiz = self._execute(
                connection,
                """
                SELECT AVG(100.0 * score / NULLIF(total, 0)) AS average
                FROM study_quiz_attempts WHERE user_id=? AND total > 0
                """,
                (self.user_id,),
            ).fetchone()
            recent_quiz = self._execute(
                connection,
                """
                SELECT * FROM study_quiz_attempts
                WHERE user_id=? ORDER BY attempted_at DESC LIMIT 1
                """,
                (self.user_id,),
            ).fetchone()
            by_section = self._execute(
                connection,
                """
                SELECT section, AVG(100.0 * score / NULLIF(total, 0)) AS average,
                       COUNT(*) AS attempts
                FROM study_quiz_attempts
                WHERE user_id=? AND total > 0 AND mode='quiz'
                GROUP BY section ORDER BY average DESC
                """,
                (self.user_id,),
            ).fetchall()
            card_events = self._execute(
                connection,
                """
                SELECT reviewed_at AS happened_at, 'card' AS kind, section, rating AS detail
                FROM study_card_review_events
                WHERE user_id=? ORDER BY reviewed_at DESC LIMIT 8
                """,
                (self.user_id,),
            ).fetchall()
            quiz_events = self._execute(
                connection,
                """
                SELECT attempted_at AS happened_at, 'quiz' AS kind, section,
                       topic || ': ' || score || '/' || total AS detail
                FROM study_quiz_attempts
                WHERE user_id=? ORDER BY attempted_at DESC LIMIT 8
                """,
                (self.user_id,),
            ).fetchall()
        activity = sorted(
            [dict(row) for row in card_events] + [dict(row) for row in quiz_events],
            key=lambda row: row["happened_at"],
            reverse=True,
        )[:8]
        average = quiz["average"] if quiz else None
        return {
            "total_reviewed": int(total_reviewed),
            "new_cards": max(0, len(all_card_ids) - int(total_reviewed)),
            "due": int(due),
            "quiz_average": float(average) if average is not None else None,
            "recent_quiz": dict(recent_quiz) if recent_quiz else None,
            "strongest": dict(by_section[0]) if by_section else None,
            "weakest": dict(by_section[-1]) if by_section else None,
